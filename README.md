...
`.github/workflows/windows.yml` builds `combosolver.exe` with MSVC on every
push and uploads it as an artifact, then runs `tools/ci/gate.py`: the
reference replay against pinned card data and scripts, with the report's
platform-invariant lines compared byte-for-byte to a committed reference
generated by a macOS build. A green gate means the Windows and macOS builds
re-execute the same duel to the same board. Regenerate the reference after
an intentional output change with `--write-reference`.
